...
export class ProjectDetector {
   ...
   * 检测并初始化项目配置
   */
  public async detectProject(workspaceFolder?: vscode.WorkspaceFolder): Promise<ProjectConfig | null> {
    try {
        ...
        if (trimmed && !trimmed.startsWith('#')) {
          ...
          if (match) {
            ...
            if (value === 'true' || value === 'false') {
              ...
              (config as any)[key] = arrayContent.split(',').map(item => item.trim().replace(/["']/g, ''));
            } else {
   ...
   * 扫描 ARB 文件
   */
  private async scanArbFiles(projectPath: string, l10nConfig: L10nConfig | null, pubspecConfig: any) {
    ...
    try {
      ...
      // 提取语言代码
      supportedLocales = arbFiles.map(file => {
