...
export class CommandHandler {
  ...
  public static registerCommands(context: vscode.ExtensionContext): void {
    ...
    const commands = [
      ...
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.editTranslation',
      ...
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.deleteTranslation',
      ...
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.showAllTranslations',
        handler.showAllTranslations
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.showStatistics',
        handler.showStatistics
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.refreshProject',
        handler.refreshProject
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.validateTranslations',
        handler.validateTranslations
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.exportTranslations',
        handler.exportTranslations
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.importTranslations',
        handler.importTranslations
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.generateMissingKeys',
        handler.generateMissingKeys
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.sortTranslations',
        handler.sortTranslations
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.duplicateTranslation',
        handler.duplicateTranslation
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.openTranslationEditor',
        handler.openTranslationEditor
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.extractToTranslation',
        handler.extractToTranslation
      ),
      vscode.commands.registerCommand(
        'flutter-i18n-vscode-inline.previewTranslation',
        handler.previewTranslation
      ),
    ];
  ...
  private editTranslation = async (key?: string, range?: vscode.Range): Promise<void> => {
    try {
      ...
      if (!key) {
        ...
        const reference = this.dartParser.getReferenceAtPosition(
          editor.document,
  ...
  private createTranslation = async (key?: string, range?: vscode.Range): Promise<void> => {
    try {
      ...
      if (otherLocales.length > 0) {
        const createOthers = await vscode.window.showQuickPick(['Yes', 'No'], {
      ...
      if (range) {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
          const codeSnippet = this.dartParser.generateReference(
            key,
            I18nPatternType.CONTEXT_L10N,
            []
          );
  ...
  private showAllTranslations = async (key?: string): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to show all translations: ${(error as Error).message}`
      );
    }
  };
  ...
  private validateTranslations = async (): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to validate translations: ${(error as Error).message}`
      );
    }
  };
  ...
  private exportTranslations = async (): Promise<void> => {
    try {
      const format = await vscode.window.showQuickPick(['JSON', 'CSV', 'Excel'], {
  ...
  private generateMissingKeys = async (): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to generate missing keys: ${(error as Error).message}`
      );
    }
  };
  ...
  private duplicateTranslation = async (key?: string): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to duplicate translation: ${(error as Error).message}`
      );
    }
  };
  ...
  private openTranslationEditor = async (): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to open translation editor: ${(error as Error).message}`
      );
    }
  };
  ...
  private extractToTranslation = async (): Promise<void> => {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to extract to translation: ${(error as Error).message}`
      );
    }
  };
  ...
  private previewTranslation = async (key?: string): Promise<void> => {
    try {
      ...
      if (translation) {
        vscode.window
          .showInformationMessage(`${this.getLocaleName(selectedLocale)}: "${translation}"`, 'Copy')
          .then(action => {
            if (action === 'Copy') {
