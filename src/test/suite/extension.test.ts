...
suite('Extension Test Suite', () => {
  ...
  suite('DartParser Tests', () => {
    ...
    test('should parse S.of(context) pattern', () => {
      ...
      const mockDocument = createMockDocument(dartCode);
      const references = dartParser.parseDocument(mockDocument);

      assert.strictEqual(references.references.length, 1);
      assert.strictEqual(references.references[0].key, 'hello');
      assert.strictEqual(references.references[0].pattern, 'generated_class');
    });
  ...
  suite('CacheManager Tests', () => {
    ...
    test('should handle cache expiration', done => {
      ...
      setTimeout(() => {
        const cachedValue = cacheManager.get(key);
        assert.strictEqual(cachedValue, null, 'Cache should expire');
        done();
      }, 150);
    });
    ...
    test('should clear cache by prefix', () => {
      ...
      assert.strictEqual(cacheManager.get('prefix:key1'), null);
      assert.strictEqual(cacheManager.get('prefix:key2'), null);
  ...
  suite('ConfigManager Tests', () => {
    ...
    test('should validate configuration', () => {
      ...
      const invalidConfig = {
        ...
        // Missing hoverPreview and diagnostics
      };
